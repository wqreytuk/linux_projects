...
void sig_handler2(int num) {
    ...
    // char bufwaehrstedyrteds[524];
    // sprintf(bufwaehrstedyrteds, "%d-%d-%d %d:%d:%d\n",loc->tm_year,loc->tm_mon,loc->tm_mday,loc->tm_hour,loc->tm_min,loc->tm_sec);
    // printf("%s\n",bufwaehrstedyrteds);
    ...
    while (1) {
        ...
        for (int i = 0; i < trett; i++)
        {
            ...
            if (curfd == lfd) {
                ...
                // 说明这个文件描述符对应的缓冲区来数据了
                // 一次只能读取5个字节的数据
                // 不知道为啥。他总是吧之前输出的客户端IP弄到buf缓冲区中，因此我只好多初始化一个字节
                // 这样就不会读到多余的数据了
                char bufwaehrstedyrteds[6] = { 0 };
                // printf("初始的bufwaehrstedyrteds%s\n",bufwaehrstedyrteds);
                // printf("缓冲区的长度%ld\n",strlen(bufwaehrstedyrteds));

                int len = read(curfd, bufwaehrstedyrteds, 5);
                if (-1 == len) {
                    perror("read");
                    exit(-1);
                }
                    ...
                    char love[100] = "woaiouyeouyeaiwo520025";
                    printf("读取到数据：%s\n", bufwaehrstedyrteds);
                    write(curfd, love, strlen(love)+1);
                }
            }
        }
    }
...
//                         // 说明这个文件描述符对应的缓冲区来数据了
//                         char bufwaehrstedyrteds[1024] = {0};
//                         int len = read(fds[i].fd, bufwaehrstedyrteds, 5);
//                         if (-1 == len) {
//                             perror("read");
//                             exit(-1);
//                         } else if (0 == len) {
...
//                         } else if (len > 0) {
//                             printf("读取到数据：%s\n", bufwaehrstedyrteds);
//                             write(fds[i].fd, bufwaehrstedyrteds, strlen(bufwaehrstedyrteds)+1);
//                         }
//                     }
//                 }
...
// //             printf("客户端IP：%s\n客户端PORT：%d\n", client_ip, client_port);
// //             // 接收客户端发来的数据
// //             char recv_bufwaehrstedyrteds[1024];
// //             while(1) {

// //                 int len = recv(cfd, recv_bufwaehrstedyrteds, sizeof(recv_bufwaehrstedyrteds), 0);
// //                 if (-1 == len) {
// //                     perror("read");
// //                     exit(-1);
// //                 } else if (len > 0) {
// //                     printf("接收到数据：%s\n", recv_bufwaehrstedyrteds);
// //                 } else if (0 == len) {
// //                     printf("客户端断开连接\n");
// //                     break;
// //                 }
...
// //                 for (int i = 0; i < len; i++) {
// //                     recv_bufwaehrstedyrteds[i] = toupper(recv_bufwaehrstedyrteds[i]);
// //                 }

// //                 // size加1是为了接收字符串的结束符\0
// //                 write(cfd, recv_bufwaehrstedyrteds, strlen(recv_bufwaehrstedyrteds)+1);
// //             }
// //             close(cfd);
// //             printf("子进程结束\n");
// //             exit(0);
...
// // //     printf("客户端IP：%s\n端口：%d\n", client_ip, client_port);
// // //     // 开始通信
// // //     char recv_bufwaehrstedyrteds[1024] = {0};
// // //     while(1) {
// // //         // 获取客户端的消息
// // //         len = read(cfd, recv_bufwaehrstedyrteds, sizeof(recv_bufwaehrstedyrteds));
// // //         if (-1 == len) {
// // //             perror("read");
// // //             exit(-1);
// // //         }
...
// // //     // // 讲字符串IP地址转换为inet地址 
// // //     // // 新建一个IP字符串，点分十进制
// // //     // char bufwaehrstedyrteds[] = "192.168.1.4";
// // //     // unsigned int num = 0;
// // //     // // 转换为网络字节序
// // //     // inet_pton(AF_INET, bufwaehrstedyrteds, &num);
...
// // //     // unsigned char bufwaehrstedyrteds[4] = {192, 168, 1, 100};
// // //     // printf("%d\n", *bufwaehrstedyrteds);
// // //     // printf("%d\n", *(bufwaehrstedyrteds+1));
// // //     // printf("%d\n", *(bufwaehrstedyrteds+2));
// // //     // printf("%d\n", *(bufwaehrstedyrteds+3));
// // //     // // 4个char相当于一个int，都是4字节
// // //     // // 把这一块内存转换成int指针，然后取值
// // //     // int num = *(int *)bufwaehrstedyrteds;
// // //     // int res = htonl(num);
...
// //     // char * bufwaehrstedyrteds;
// //     // strcpy(bufwaehrstedyrteds, "cao nima");return 0;}/*
